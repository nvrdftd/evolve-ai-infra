...
resource "aws_subnet" "public" {
  ...
  tags = merge(
    var.tags,
    {
      Name = "${var.vpc_name}-public-sn-${local.azs[count.index]}"
...
resource "aws_subnet" "private" {
  ...
  tags = merge(
    var.tags,
    {
      Name = "${var.vpc_name}-private-sn-${local.azs[count.index]}"
...
resource "aws_route" "private_nat_gateway" {
  ...
  destination_cidr_block = "0.0.0.0/0"
  nat_gateway_id         = aws_nat_gateway.main[var.single_nat_gateway ? 0 : min(count.index, length(aws_nat_gateway.main) - 1)].id
}
